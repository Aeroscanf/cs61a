...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if place.ant.is_container and self.is_container == False:
                ...
                place.ant = self
                self.place = place
                if place.ant.can_contain(other):
                    place.ant.store_ant(other)
                    return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, min_range, max_range):
        place = self.place
        distance = 0
        while place is not None and distance < min_range:
            place = place.entrance  # Move to the next Place
            distance += 1
        
        # Now check for bees in the remaining range
        # print(f"place = {place}, distance={distance}")
        while place is not None and distance <= max_range and place.is_hive is False:
            if place.bees:
                return random_bee(place.bees)  # Return a random bee from this place
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        if self.ant_contained == None and isinstance(other, ContainerAnt) == False:
            return True
        return False
        # END Problem 8
    ...
    def action(self, gamestate):
        ...
        # return super().action(self.ant_contained)
        # END Problem 8
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.queenant is None:
            queen = super().construct(gamestate)
            gamestate.queenant = queen
            return queen
        return None
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place is not None:
            if place.ant and place.ant is not gamestate.queenant:
                ant = place.ant
                if ant.double_damage is False:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        #change the bee direction
        if self.back_time and self.place.entrance.is_hive is False:
            destination = self.place.entrance

        # Extra credit: Special handling for bee direction
        #slow bee when gamestate.time is odd
        if self.slow_turn > 0 and gamestate.time % 2:
            self.slow_turn -= 1
            # print(f'slow_turn = {self.slow_turn}')
            return
        if self.slow_turn > 0: 
            self.slow_turn -= 1
        ...
        if self.blocked():
            ...
            if self.back_time > 0: self.back_time -= 1
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2, queenant = None):
