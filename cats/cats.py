...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def func(texts):
        texts = split(lower(remove_punctuation(texts)))
        #print(texts)
        for t in topic:
...
def accuracy(typed, reference):
    ...
    if len_reference_words == 0 and len_typed_words == 0:
        return 100.0
    if len_typed_words == 0 or len_reference_words == 0:
        return 0.0
    
    correct = 0.0
    for i in range(len(typed_words)):
        if i >= len_reference_words:
            break
        if typed_words[i] == reference_words[i]:
            correct += 1.0
        
    return correct / len_typed_words * 100.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    typed_len = len(typed) / 5.0
    elapsed = elapsed / 60.0
    return typed_len / elapsed
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if word == typed_word:
            return word
        diff[word] = diff_function(typed_word, word, limit)
    sorted_diff = sorted(diff.items(), key=lambda item: item[1])

    if sorted_diff[0][1] > limit:
        return typed_word
    else:
...
def sphinx_swaps(start, goal, limit):
    ...
    # Count substitutions only if within limit
    if start[0] != goal[0]:
        if limit > 0:
            return 1 + sphinx_swaps(start[1:], goal[1:], limit - 1)
        else:
            return float('inf')  # Exceeded limit
...
def minimum_mewtations(start, goal, limit):
    ...
    # Base case: if start is empty, we need to add all characters of goal
    if len(start) == 0:
        return len(goal)
    
    # Base case: if goal is empty, we need to remove all characters of start
    if len(goal) == 0:
        return len(start)
    
    # If we've reached the limit of edits allowed, return a large number
    if limit < 0:
        return float('inf')    

    # If the first characters are equal, move to the next characters without any cost
    if start[0] == goal[0]:
        return minimum_mewtations(start[1:], goal[1:], limit)

    # If the first characters are different, calculate costs for each operation
    add = minimum_mewtations(start, goal[1:], limit - 1) + 1  # Insert character into start
    remove = minimum_mewtations(start[1:], goal, limit - 1) + 1  # Remove character from start
    substitute = minimum_mewtations(start[1:], goal[1:], limit - 1) + 1  # Substitute character

    # Return the minimum cost of operations if within limit; otherwise return a large number
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    # Upload progress using provided function
...
def time_per_word(words, times_per_player):
    ...
    for times_player in times_per_player:
        time = []
        for i in range(len(times_player)-1):
            time.append(times_player[i+1] - times_player[i])
        times.append(time)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    # Initialize a list to hold the fastest words for each player
    fastest = [[] for _ in player_indices] ####
    
    # Iterate over each word index
    for word_index in word_indices:
        # Find the index of the player who typed this word fastest
        min_time = float('inf') ####
        ...
        # Add the word to the corresponding player's fastest words list
        fastest[fastest_player_index].append(match["words"][word_index])
