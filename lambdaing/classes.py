...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        player.hand + [player.draw() for _ in range(2)]
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if player.hand:
            ...
            for i in range(len(player.hand)):
                if player.hand[i].attack - player.hand[i].defense > max:
                    max = player.hand[i].attack - player.hand[i].defense
                    index = i
            best_card = player.hand.pop(i)
