...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        map_number = opponent_score
        res = player_score
        while map_number != 0:
            tmp = map_number % 10
            map_number //= 10
            res = digit_fn(tmp)(res)
        res = res % 30
        return res
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:  # Player 0's turn
            ...
            if hog_pile(score0, score1):  # Check Hog Pile rule
                score0 += hog_pile(score0, score1)

        else:  # Player 1's turn
            num_rolls = strategy1(score1, score0)
            score1 += take_turn(num_rolls, score1, score0, dice, goal)
            if hog_pile(score1, score0):  # Check Hog Pile rule
        ...
        if message is not None: 
            ...
            pass
        if score0 >= goal or score1 >= goal:  # Check if Player 0 er Player 1 wins
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if current_leader != last_leader:
        return current_leader, f"Player {current_leader} takes the lead by {diff}"
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    scor = hefty_hogs(score, opponent_score)
    if scor >= threshold:
        return 0
    return num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    scor = score + hefty_hogs(score, opponent_score)
    if hog_pile(scor, opponent_score) != 0:
        return 0
